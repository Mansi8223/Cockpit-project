...
function ClientActivity({item}) {
    ...
    const uploadHandler=()=>{
        ...
        fetch(`http://34.209.233.51/api/client/add-deliverable/${item.task}`, requestOptions)
        .then(response => response.text())
        .then(result => {
            setLoading(false)
  ...
  return (
    <>
        {loading?<Loader loading={loading}/>:<div className={`d-flex d-flex-row d-align-start d-justify-space-between p-4 bg-white box-s border-rounded-12 mt-6`}>
